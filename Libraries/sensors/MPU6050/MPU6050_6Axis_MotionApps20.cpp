...
//dubious section
void MPU6050::start(int8_t Xoffset, int8_t Yoffset, int8_t Zoffset, int16_t Zacceloffset){
    initialize();
    devStatus = dmpInitialize();

    //set offsets
    setXGyroOffset(Xoffset);
    setYGyroOffset(Yoffset);
    setZGyroOffset(Zoffset);
    setZAccelOffset(Zacceloffset);
    offsetforreset = 0;
    //??
    ...
    if (devStatus == 0){
        setDMPEnabled(true);
        //attachInterrupt(0, dmpDataReady, RISING) -- arduino function
...
//    else if (mpuIntstatus & 0x02)
    ...
    return euler[0] * 180/M_PI - offsetforreset;
}

//void MPU6050::scamreset(){
//    offsetforreset = getangle() + offsetforreset;
//}
